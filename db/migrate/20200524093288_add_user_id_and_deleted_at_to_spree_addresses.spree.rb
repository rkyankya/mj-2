...
class AddUserIdAndDeletedAtToSpreeAddresses < ActiveRecord::Migration[5.2]
  def change
    unless column_exists?(:spree_addresses, :user_id)
      add_column :spree_addresses, :user_id, :integer
    end
    unless index_exists?(:spree_addresses, :user_id)
      add_index :spree_addresses, :user_id
    end

    unless column_exists?(:spree_addresses, :deleted_at)
      add_column :spree_addresses, :deleted_at, :datetime
    end
    unless index_exists?(:spree_addresses, :deleted_at)
